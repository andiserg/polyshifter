...
class Point:
    ...
    def __init__(self, x: float | Fraction, y: float | Fraction) -> None:
        self.x = Fraction(str(x))
        self.y = Fraction(str(y))

    def distance_to(self, other: "Point") -> Fraction:
        dx = self.x - other.x
        dy = self.y - other.y
        return Fraction(Fraction(dx * dx + dy * dy) ** Fraction(1, 2))

    def get_normalized(self) -> "Point":
...
class Polygon:
    ...
    def __init__(self, points_coords: list[tuple]) -> None:
